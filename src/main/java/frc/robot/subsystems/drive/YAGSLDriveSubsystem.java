...
/**
 * YAGSLDriveSubsystem manages the swerve drive using YAGSL (Yet Another Generic
 * Swerve Library).
 *
 ...
 * - Disables heading correction and cosine compensation in simulation for
 * realism
 */
@Logged
public class YAGSLDriveSubsystem extends SubsystemBase implements AutoCloseable {
    ...
    private volatile int lastTagCount = 0;
    private volatile double lastAvgTagDist = 0.0;
    private volatile boolean lastVisionValid = false;
    ...
    public static final double MAX_DISTANCE_FOR_VISION = 6.0; // meters (reject beyond this)
     ...
     *
     * @param visionSubsystems One or more vision subsystems for AprilTag-based
     *                         localization
     */
    public YAGSLDriveSubsystem(VisionSubsystem... visionSubsystems) {
        ...
        // This runs asynchronously to avoid blocking the main thread with NetworkTables
        // I/O
     ...
     * @param fieldRelative If true, translation is relative to field. If false,
     *                      relative to robot.
     */
     ...
     * @param fieldRelative         If true, velocities are relative to field. If
     *                              false, relative to robot.
     */
    ...
    public void driveFieldRelative(ChassisSpeeds chassisSpeeds) {
        SmartDashboard.putNumber("CommandedX", chassisSpeeds.vxMetersPerSecond);
        SmartDashboard.putNumber("CommandedY", chassisSpeeds.vyMetersPerSecond);
        swerveDrive.driveFieldOriented(chassisSpeeds);
    }

    /**
     * Drives the robot using robot-relativeChassisSpeeds.
     *
     * @param chassisSpeeds Desired chassis speeds
     */
     ...
     *
     * @return The current estimated pose (includes wheel odometry and vision
     *         fusion)
     */
     ...
     * - Blue alliance (facing away from blue DS): heading = 0° (facing X+, toward
     * red wall)
     * - Red alliance (facing away from red DS): heading = 180° (facing X-, toward
     * blue wall)
     *
     * The robot is always positioned facing away from the driver station at match
     * start.
     * This method accounts for alliance color to set the correct heading in blue
     * origin coordinates.
     *
     * In YAGSL 2025.7.1+, gyro zeroing is no longer automatic at startup.
     * Call this method in autonomousInit() to ensure proper odometry
     * initialization.
     */
     ...
     * 4. Adds vision measurements to pose estimator with appropriate standard
     * deviations
     *
     * Runs on background thread via Notifier to avoid blocking main thread with
     * NetworkTables I/O.
     * Processes all vision subsystems to enable multi-camera setups for redundancy
     * and wider coverage.
     */
        ...
        try {
            ...
            for (VisionSubsystem visionSubsystem : visionSubsystems) {
                ...
                if (estimate.isPresent()) {
                    ...
                    if (est.tagCount > 0 && est.avgTagDist <= MAX_DISTANCE_FOR_VISION) {
            ...
            this.lastTagCount = totalTagCount;
            this.lastAvgTagDist = validEstimates > 0 ? totalAvgTagDist / validEstimates : 0.0;
            this.lastVisionValid = validEstimates > 0;
     ...
     * - MANUAL_DRIVE: Joystick active → record home pose (once), then drive
     * manually
     ...
     * @return Command that alternates between manual drive and return-to-home
     *         pathfinding
     */
    public Command returnToHomeCommand(DoubleSupplier xSupplier,
            DoubleSupplier ySupplier,
            DoubleSupplier rotSupplier) {
        // State variable: home pose (captured in closure, persists across command
        // executions)
    ...
    public Command debugDriveCommand(CommandXboxController driverXbox) {
        // Applies deadbands and inverts controls because joysticks
        // are back-right positive while robot
        // controls are front-left positive
        // left stick controls translation
        // right stick controls the desired angle NOT angular rotation
        Command driveFieldOrientedDirectAngle = this.driveCommand(
                () -> MathUtil.applyDeadband(driverXbox.getLeftY(), OIConstants.kDriveDeadband),
                () -> MathUtil.applyDeadband(driverXbox.getLeftX(), OIConstants.kDriveDeadband),
                () -> driverXbox.getRightX(),
                () -> driverXbox.getRightY());

        return driveFieldOrientedDirectAngle;
    }
     ...
     * @return true if all inputs are within deadband (|input| <
     *         OIConstants.kDriveDeadband)
     */
    private boolean isInDeadband(double x, double y, double rot) {
        return Math.abs(x) < OIConstants.kDriveDeadband && Math.abs(y) < OIConstants.kDriveDeadband
                && Math.abs(rot) < OIConstants.kDriveDeadband;
    }
    ...
    /**
     * Configures PathPlanner's AutoBuilder for autonomous pathfinding and path
     * following.
    ...
    private void configurePathPlanner() {
        try {
            ...
            Preferences.initDouble("PathPlanner/Rotation P", 0.04);
            Preferences.initDouble("PathPlanner/Rotation I", 0.0);
            Preferences.initDouble("PathPlanner/Rotation D", 0.01);
            ...
            PIDConstants rotationPID = new PIDConstants(
                    Preferences.getDouble("PathPlanner/Rotation P", 0.4),
                    Preferences.getDouble("PathPlanner/Rotation I", 0.0),
                    Preferences.getDouble("PathPlanner/Rotation D", 0.01));
            ...
            AutoBuilder.configure(
                    ...
                    this::driveRobotRelative, // Consumer<ChassisSpeeds>: Drive with chassis speeds
